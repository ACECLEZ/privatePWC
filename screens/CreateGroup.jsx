...
const CreateGroup = ({navigation}) => {
    ...
    const handleSubmit = async () => {
        await addDoc(collection(db, "chats"), {
            users: [auth.currentUser.email, ...emails],
            messages: [],
            isGroup: true,
            groupName
        })
    ...
    return (
        <SafeAreaView>
            <View style={styles.content}>
                ...
                <View style={styles.tagContainer}>
                    {
                        emails.map((email, index) => (
                            <View style={styles.tags} key={index}>
                                ...
                                <TouchableOpacity style={styles.tagDelete}
                                                  onPress={() => setEmails(emails.filter(em => em !== email))}>
                                    <Ionicons name="close-circle-outline" color={colors.secondaryColor} size={20}/>
                                </TouchableOpacity>
                            </View>
                        ))
                    }
                ...
                <TextInput keyboardType="default" style={styles.input} placeholder="Group Name"
                           onChangeText={text => setGroupName(text)}/>

                <TouchableOpacity style={styles.btn} activeOpacity={.7} onPress={handleSubmit}>
                    <Ionicons name="md-chatbubble-outline" color={colors.secondaryColor} size={16}/>
                    <Text
                        style={styles.btnText}>{emails.length > 0 ? `Start Chat With ${emails.length} People` : 'Start'}</Text>
                </TouchableOpacity>
            </View>
        </SafeAreaView>
    )
